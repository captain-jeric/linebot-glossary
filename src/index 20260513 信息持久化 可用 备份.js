...
function isCustomerUsable(customer) {
  ...
  if (Number(customer.used_chars || 0) >= Number(customer.quota_chars || 0)) {
...
function getRemainingChars(customer) {
  return Math.max(0, Number(customer?.quota_chars || 0) - Number(customer?.used_chars || 0));
}
...
function normalizeCustomerInput(body) {
  const quotaChars = Math.max(0, Number.parseInt(body.quota_chars || "0", 10) || 0);
  const usedChars = Math.max(0, Number.parseInt(body.used_chars || "0", 10) || 0);
  ...
  return {
    ...
    quota_chars: quotaChars,
    used_chars: usedChars,
    expires_at: String(body.expires_at || "").trim(),
...
function validateCustomerInput(input) {
  ...
  if (!input.expires_at || Number.isNaN(new Date(input.expires_at).getTime())) {
    return "到期时间格式不正确，例如：2026-06-12 23:59:59+07";
  }
    ...
    .map((customer) => {
      ...
      const usagePercent =
        Number(customer.quota_chars || 0) > 0
          ? Math.min(100, Math.round((Number(customer.used_chars || 0) / Number(customer.quota_chars || 0)) * 100))
          : 0;
      ...
      return `<section class="customer">
        <form method="post" action="/admin/customers/${escapeHtml(customer.id)}">
          ...
          <div class="grid">
            ...
            <label>状态
              <select name="status">
                ${["active", "trial", "paused", "expired", "cancelled"]
                  .map(
                    (status) =>
            ...
            <label>到期时间<input name="expires_at" value="${escapeHtml(customer.expires_at)}"></label>
            <label>总额度<input name="quota_chars" type="number" min="0" step="1" value="${escapeHtml(customer.quota_chars)}"></label>
            <label>已用额度<input name="used_chars" type="number" min="0" step="1" value="${escapeHtml(customer.used_chars)}"></label>
            <label class="check"><input name="activation_code_enabled" type="checkbox" ${customer.activation_code_enabled ? "checked" : ""}> 激活码启用</label>
            <label>备注<input name="notes" value="${escapeHtml(customer.notes || "")}"></label>
          </div>
          <div class="meter"><span style="width:${usagePercent}%"></span></div>
          <p class="meta">
            剩余 ${formatNumber(remaining)} 字符 · 绑定 ${item.count} 个聊天环境 · 开启 ${item.enabledCount} 个 · 最后活跃 ${escapeHtml(formatDate(item.lastActiveAt))}
          </p>
        </form>
      </section>`;
    })
...
<head>
  ...
  <style>
    ...
    form { margin: 0; }
    .panel, .customer { background: #fff; border: 1px solid #d9e0ea; border-radius: 8px; padding: 16px; margin-bottom: 14px; }
    ...
    .customer-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 12px; }
    .customer-head div { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
    ...
    <section class="panel">
      ...
      <form method="post" action="/admin/customers">
        ...
        <div class="grid">
          ...
          <label>到期时间<input name="expires_at" placeholder="2026-06-12 23:59:59+07"></label>
          <label>总额度<input name="quota_chars" type="number" min="0" step="1" value="100000"></label>
          <label>已用额度<input name="used_chars" type="number" min="0" step="1" value="0"></label>
...
async function chargeCustomerUsage(customerId, chargedChars) {
  ...
  const nextUsed = Number(customer.used_chars || 0) + chargedChars;
  if (nextUsed > Number(customer.quota_chars || 0)) return false;

  const { error: updateError } = await supabase
    .from("customers")
    .update({ used_chars: nextUsed, updated_at: new Date().toISOString() })
    .eq("id", customerId);
...
async function handleEvent(event) {
  ...
  const chargeMultiplier = state.activation.mode === "trilingual" ? 2 : 1;
  const chargedChars = countChargeableChars(text) * chargeMultiplier;
  ...
  const messages =
    state.activation.mode === "trilingual"
      ? await buildTrilingualMessages(text, sourceLang)
      : await buildBilingualMessages(text, sourceLang, state.activation);
  ...
  return replyMessages(event, messages);
}
...
function buildUsageText(customer) {
  ...
  return [
    "当前激活码用量",
    "",
    `总额度：${formatNumber(customer.quota_chars)} 字符`,
    `已使用：${formatNumber(customer.used_chars)} 字符`,
    `剩余额度：${formatNumber(getRemainingChars(customer))} 字符`,
    `到期时间：${formatDate(customer.expires_at)}`,
  ].join("\n");
}
